...
BEGIN
  ...
  SELECT
    ...
    GREATEST(
      0,
      COALESCE(
        (CURRENT_DATE - MAX(CASE WHEN d.units_total > 0 THEN d.report_date END))::INTEGER,
        (CURRENT_DATE - MIN(d.report_date))::INTEGER,
        0
      )
...
CREATE OR REPLACE FUNCTION refresh_products_from_daily_skus(
  p_user UUID,
  p_skus TEXT[],
  p_marketplace TEXT DEFAULT NULL
)
